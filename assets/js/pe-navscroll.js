...
var peNavScroll = (function(){
  ...
  var initialize = function (options) {
    ...
    var tMenu = document.querySelector(config.nav),
        ...
        navLinks = tMenu.querySelectorAll("a"),
        i = links.length,
        lId;


    var scrlItem = [].slice.call(navLinks).map(function(value, index){
      var item = document.getElementById(/[^#]+$/.exec(navLinks[index].href)[0]);
      if(item !== undefined) { return item; }
    })

    while (i--) {
      if( document.getElementById(/[^#]+$/.exec(links.item(i).href)[0])){

        links.item(i).addEventListener("click", function(e) {

          var startPos = root.scrollTop;
          var maxScroll = root.scrollHeight - window.innerHeight;
          var endPos = document.getElementById(/[^#]+$/.exec(this.href)[0]).getBoundingClientRect().top - (tMenu.offsetHeight + config.offset);
